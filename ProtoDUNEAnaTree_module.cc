////////////////////////////////////////////////////////////////////////
// Class:       ProtoDUNEAnaTree
// File:        ProtoDUNEAnaTree_module.cc
//
...
//
// Updated includes to reflect the new structure of dunetpc
...
class protoana::ProtoDUNEAnaTree : public art::EDAnalyzer
{
  public:

    explicit ProtoDUNEAnaTree(fhicl::ParameterSet const & p);

    ProtoDUNEAnaTree(ProtoDUNEAnaTree const &) = delete;
    ProtoDUNEAnaTree(ProtoDUNEAnaTree &&) = delete;
    ProtoDUNEAnaTree & operator = (ProtoDUNEAnaTree const &) = delete;
    ProtoDUNEAnaTree & operator = (ProtoDUNEAnaTree &&) = delete;
...
protoana::ProtoDUNEAnaTree::ProtoDUNEAnaTree(fhicl::ParameterSet const & p)
  :
  EDAnalyzer(p),
...
void protoana::ProtoDUNEAnaTree::beginJob()
{
...
void protoana::ProtoDUNEAnaTree::analyze( art::Event const & evt )
{
...
bool protoana::ProtoDUNEAnaTree::EventSelector( art::Event const & evt, std::vector< const recob::PFParticle* > & PrimaryPFParticles )
{ // Let's have some logic to select our events
  ...
  if ( doEvent ) {
    ...
    if ( !evt.isRealData() ) { // MC
      ...
      if (candidates.electron) {
        std::cout << "Electron candidate" << std::endl;
  ...
  if ( doEvent ) {
    ...
    const recob::Track*  thisTrack  = pfpUtil.GetPFParticleTrack(  *primary, evt, fPFParticleLabel, fTrackLabel );
    const recob::Shower* thisShower = pfpUtil.GetPFParticleShower( *primary, evt, fPFParticleLabel, fShowerLabel  );
    if ( thisShower != nullptr ) {
      std::cout << "This event primary is a shower" << std::endl;
      anaTree_nDaughters = primary->NumDaughters();
      doEvent = true;
    }
    else if ( thisTrack != nullptr ) {
      //std::cout << "This event primary is a track" << std::endl;
      //std::cout << "It has " << primary->NumDaughters() << std::endl;
      if ( primary->NumDaughters() == 1 ) {
        for( const int daughterID : primary->Daughters() ) {
          const recob::PFParticle* daughter = &(recoParticles->at(daughterID));
          const recob::Shower* thisDaughterShower = pfpUtil.GetPFParticleShower( *daughter, evt, fPFParticleLabel, fShowerLabel  );
          if ( thisDaughterShower != nullptr ) {
            std::cout << "The daughter is a shower" << std::endl;
            doEvent = true;
            doEvent = false;
          }
          else {
            //std::cout << "The daughter is NOT a shower" << std::endl;
            doEvent = false;
          }
        }
      }
      ...
      //std::cout << "The primary is not a shower or a track" << std::endl;
      if ( primary->NumDaughters() == 1 ) {
        for( const int daughterID : primary->Daughters() ) {
          const recob::PFParticle* daughter = &(recoParticles->at(daughterID));
          const recob::Shower* thisDaughterShower = pfpUtil.GetPFParticleShower( *daughter, evt, fPFParticleLabel, fShowerLabel  );
...
void protoana::ProtoDUNEAnaTree::FillPFParticle( art::Event const & evt,
                                                 art::ValidHandle< std::vector< recob::PFParticle > > & recoParticles,
                                                 const recob::PFParticle* particle,
                                                 int flowLevel )
{
...
void protoana::ProtoDUNEAnaTree::FillShower( art::Event const & evt, const recob::Shower* thisShower, const recob::PFParticle* particle )
{
...
double protoana::ProtoDUNEAnaTree::dEdx5cm( art::Event const & evt, const recob::Shower* thisShower, std::vector<double> xVec, std::vector<double> yVec, std::vector<double> zVec, std::vector<double> dEdxVec, std::vector<double> pitchVec )
{
...
double protoana::ProtoDUNEAnaTree::MydEdx( std::vector<double> xVec, std::vector<double> yVec, std::vector<double> zVec, std::vector<double> eVec )
{
...
void protoana::ProtoDUNEAnaTree::oldProjLengthFinder( std::vector<double> xVec, std::vector<double> yVec, std::vector<double> zVec ) 
...
double protoana::ProtoDUNEAnaTree::transToPCA( TVector3 point, TVector3 cent, TVector3 axis )
{
...
void protoana::ProtoDUNEAnaTree::showerStartFinder( std::vector<double> xVec, std::vector<double> yVec, std::vector<double> zVec, std::vector<double> eVec )
{
...
std::vector<double> protoana::ProtoDUNEAnaTree::makeBinEdges( double binWidth, double maxZ )
...
std::pair< std::vector< std::vector<TVector3> >, std::vector< std::vector<double> > > protoana::ProtoDUNEAnaTree::binShower( std::vector<double> binEdges, std::vector<TVector3> xyzVec, std::vector<double> eVec )
...
std::vector< std::vector<double> > protoana::ProtoDUNEAnaTree::binStdCalc( std::vector< std::vector<TVector3> > binnedPoints )
...
std::vector<double> protoana::ProtoDUNEAnaTree::xyBinStdCalc( std::vector< std::vector<double> > binStds )
...
double protoana::ProtoDUNEAnaTree::startFinder( std::vector< double > xyBinStds, double threshold )
...
std::vector<double> protoana::ProtoDUNEAnaTree::dEdtCalc( std::vector< std::vector<double> > binnedEnergies, float binWidth )
{
...
std::tuple< TVector3, TVector3, TVector3, TVector3, TVector3 > protoana::ProtoDUNEAnaTree::runPCA( std::vector<TVector3> pointVector, const recob::Shower* thisShower )
...
std::tuple< TVector3, TVector3, TVector3, TVector3, TVector3 > protoana::ProtoDUNEAnaTree::recursivePCA( std::vector<TVector3> pointVector, const recob::Shower* thisShower, std::vector<double> weightVector, double prevAveWeight, int recursionLevel )
{ // Pretty much copied from Pandora
...
void protoana::ProtoDUNEAnaTree::showerEnergyEstimationMethods( art::Event const & evt, const recob::Shower* thisShower )
{

    // We will need the particle to have hits with spacepoints so lets count the number of hits spacepoints
  int nHitsWithSpacePoints = 0;
  ...
  for ( art::Ptr<recob::Hit> hit : hits ) {
    ...
    // Lets not do this event
  if (nHitsWithSpacePoints == 0) {
...
std::pair<TVector3, double> protoana::ProtoDUNEAnaTree::xyzSpatialAndPitch( art::Ptr<recob::Hit> hit, const recob::Shower* thisShower, TVector3 spoint )
{
...
std::pair<double,double> protoana::ProtoDUNEAnaTree::dQcorrections( TVector3 pos, double dQ )
{
...
void protoana::ProtoDUNEAnaTree::missingEnergy( art::Event const & evt, const std::vector<art::Ptr<recob::Hit>> showerHits )
{
...
double protoana::ProtoDUNEAnaTree::GetYPos( double z, std::vector<TVector3> spointVec )
{
...
void protoana::ProtoDUNEAnaTree::sandbox( art::Event const & evt, std::vector<art::Ptr<recob::Hit>> showerHits, std::vector<art::Ptr<recob::Hit>> mcHits, std::vector<art::Ptr<recob::Hit>> allHits, const simb::MCParticle* mcParticle )
...
void protoana::ProtoDUNEAnaTree::FillBeamData( art::Event const & evt )
{
...
void protoana::ProtoDUNEAnaTree::FillConfigTree()
{
...
const std::vector<const recob::Hit*> protoana::ProtoDUNEAnaTree::GetRecoShowerHitsFromPlane(const recob::Shower &shower, art::Event const &evt, unsigned int planeID ) const
{
...
void protoana::ProtoDUNEAnaTree::endJob(){
...
void protoana::ProtoDUNEAnaTree::InitialiseBeamInfo()
{
...
void protoana::ProtoDUNEAnaTree::InitialiseAnaTree()
{
    // Pass Checks
  NEWPCAPass  = false;
  NEWDEDXPass = false;
...
DEFINE_ART_MODULE(protoana::ProtoDUNEAnaTree)
