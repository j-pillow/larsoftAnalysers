...
// Get the hits associated to the PFParticle but in an art ptr
const std::vector< art::Ptr< recob::Hit > > protoana::ProtoDUNEPFParticleUtils::GetPFParticleHitsArtPtr( const recob::PFParticle &particle, art::Event const &evt, const std::string particleLabel) const
{
...
// Get the hits associated to the PFParticle for a plane
const std::vector<const recob::Hit*> protoana::ProtoDUNEPFParticleUtils::GetPFParticleHitsFromPlane(const recob::PFParticle &particle, art::Event const &evt, const std::string particleLabel, const unsigned int planeID) const{
...
//    std::cout << "Cluster has " << clusterHits.size() << " hits" << std::endl;
    for(auto hit : clusterHits){
      if ( hit.get()->WireID().asPlaneID().Plane == planeID ) {
        pfpHits.push_back(hit.get());
      }
    }
  }
...
// Get the number of hits
unsigned int protoana::ProtoDUNEPFParticleUtils::GetNumberPFParticleHitsFromPlane(const recob::PFParticle &particle, art::Event const &evt, const std::string particleLabel, const unsigned int planeID) const{

  return GetPFParticleHitsFromPlane(particle,evt,particleLabel,planeID).size();
